...
const userSignup = async (req, res, next) => {
    ...
    if (!error.isEmpty()) {
        return next(
            new HttpError(
                'Invalid data entered, please enter data correctly',
                422
            )
        );
    }
    ...
    try {
        ...
        return next(
            new HttpError('Signed Up failed, please try again !', 500)
        );
    }
    ...
    if (hasUser) {
        return next(
            new HttpError(
                'Signed Up failed, user already exists.',
                422
            )
        );
    }
    ...
    try {
        ...
        return next(
            new HttpError(
                'Signing up failed, please try again lataer',
                500
            )
        );
    }
...
const userLogin = async (req, res, next) => {
    ...
    try {
        isValidUser = await bcrypt.compare(
            password,
...
const newPassword = async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new HttpError('Session expired, please try again !', 422)
        );
    }
